...
import { createPrinterOrder } from "@/lib/order-service"

interface CreatePrinterOrderProps {
  isOpen: boolean
  onClose: () => void
  onOrderCreated: () => void
}

export function CreatePrinterOrder({ isOpen, onClose, onOrderCreated }: CreatePrinterOrderProps) {
  ...
  const addModel = () => {
    setFormData((prev) => ({
      ...prev,
      models: [...prev.models, { modelNo: "", name: "", price: "", qty: "1" }],
    }))
  }
  ...
  const calculateSubtotal = () => {
    return formData.models
      .reduce((total, model) => {
        const price = Number.parseFloat(model.price) || 0
        const qty = Number.parseInt(model.qty) || 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await createPrinterOrder({
        date: formData.date,
        ...
        models: formData.models.map((model, index) => ({
          id: `#${index + 1}`,
          modelNo: model.modelNo,
          name: model.name,
          price: `${model.price}DT`,
          qty: model.qty,
          total: `${(Number.parseFloat(model.price) * Number.parseInt(model.qty)).toFixed(2)}DT`,
        })),
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Create Printer Order</DialogTitle>
          <DialogDescription>Fill in the details to create a new printer order.</DialogDescription>
        </DialogHeader>
          ...
          <Tabs defaultValue="general" className="w-full">
            ...
            <TabsContent value="models">
              <div className="space-y-4 py-4">
                ...
                <Button type="button" variant="outline" onClick={addModel} className="w-full">
                  Add Model
                </Button>
              </div>
            </TabsContent>
